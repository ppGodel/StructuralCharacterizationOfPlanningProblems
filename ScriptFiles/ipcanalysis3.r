...
clas1=allplanners(nx="D" ,ny="Time", data=compresultsgraphsolved)
clas2=allplanners(nx="DM",ny="Time", data=compresultsgraphsolved)
clas3=allplanners(nx="TN",ny="Time", data=compresultsgraphsolved)
clas4=allplanners(nx="TE",ny="Time", data=compresultsgraphsolved)
clas5=allplanners(nx="TME",ny="Time", data=compresultsgraphsolved)

allclass=rbind(clas1,clas2,clas3,clas4,clas5)
boxplot(R2~Cresp+Cfactor, data=allclass)
#hist(allclass[allclass$Cfactor=="D",]$R2)
ggplot(data = allclass[allclass$R2>=0.9&allclass$Dist>0,], aes(x=factor(Cfactor), y=log(Dist+1))) + geom_violin(fill="orange", color="red") + geom_boxplot(width=0.1, fill="blue", color="white", lwd=1) + theme(text = element_text(size=30))#+facet_wrap(~dom)

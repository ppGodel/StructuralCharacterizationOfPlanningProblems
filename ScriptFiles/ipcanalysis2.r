...
#  lines(lm.model, col="red")
  ...
  xpol=c(1:length(slope.lower),length(slope.upper):1)
  ypol=c(slope.lower, rev(slope.upper))
  polygon(x=xpol, y=ypol, border=NA, col="antiquewhite2")
  lines(y.fit2, col = 'black', lwd = 2)
  lines(slope.upper, col = 'brown', lty = 2, lwd = 2)
  lines(slope.lower, col = 'brown', lty = 2, lwd = 2)
  #return(bands)
}
...
for(p in 1:length(apl)){
    ...
    par(mar=c(5,5,3,9),xpd=FALSE)
    plot(0,type='n', xlim=c(0,log(maxTN)), ylim=c(0,log(maxT)), xlab="Nodes", ylab="Time", main=paste0("Planner: ", apl[p]) )
    reg.conf.intervals(log(auxres$TN),log(auxres$Time+1))
    for(c in 1:length(cl)){
        ...
        for(d in 1:length(dl)){
            ...
            ni=match(an,labls)
            dcaux=c(ni,dcaux)
            points(log(Time+1)~log(TN), col=colores[ni], pch=19, data=ares[ares$Dom==dl[d],])
        }
    }
    par(xpd=TRUE)
#    legend(maxTN*1.06, log(maxT)*0.6, legend=apl, pch=pchs)
    legend(log(maxTN)*1.06, log(maxT)*1, legend=dlaux,col=colores[dcaux], pch=19)
    imprimirfin()
}
...
for(p in 1:length(apl)){
    ...
    #alm=lm(log(Time+1)~log(TE),data=auxres)
    #abline(alm)
    reg.conf.intervals(log(auxres$TE),log(auxres$Time+1))

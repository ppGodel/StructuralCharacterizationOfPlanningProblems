...
for(j in 1:nrow(graphsds)){
    ...
    if(dim(ag)[1]>0){
        ...
        if(gtype=="l"){
            ...
            plot(TFNMN~T, data=ag, type='l', ylim=c(0, max(limval$MN,1)), main=wg$gn)
            ...
            if(xtype=="l"){
                ...
                imprimirini(typ=typu,name=paste0("matrix/Log/nodeDistLog", wg$gn),12,7.25)
                par(mar=c(3,3,3,9),xpd=FALSE)
                ...
                zona <- matrix(rep(0, (2 * vert * (hor+1))), nrow = vert, ncol = 2 * (hor+1))
                for(l in 0:(hor)){
                ...
                imprimirini(typ=typu,name=paste0("matrix/raw/nodeDist", wg$gn),12,7.25)
                par(mar=c(3,3,3,9),xpd=FALSE)
                image(t(zona), col=c("white", colpal), xlim=c(-0.05,1.05), xaxt='n', yaxt='n', main=wg$gn)
                axis(side=1,at=(seq(0,hor,hor/5)/hor), labels=round(seq(0,hor,hor/5)))
